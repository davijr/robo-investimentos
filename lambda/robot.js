...
export class RobotService {
  ...
  getBuyBuySell(allSymbols: any, buySymbols: any) {
  ...
  getBuySellSell(allSymbols: any, buySymbols: any) {
    const buyBuySell: any[] = []
        ...
        if (settings.robotStatus === RobotStatusEnum.SEARCHING) {
          ...
          if (crossRate >= settings.profitability && p1 && p2 && p3) {
            ...
            if (![RobotStatusEnum.ERROR, RobotStatusEnum.TRADING].includes(settings.robotStatus)
              ...
              await this.setRobotStatus(RobotStatusEnum.TRADING);
              // NotificationService.playSound(NotificationSoundType.FOUND);
              const oportunidade = oportunityService.create({ strategy: 'BSS', profitability: crossRate, initialValue: settings.amount, ordersRequest: operations });
  ...
  applyValidations(valueParam: number, filters: any, filterName: 'PRICE_FILTER' | 'LOT_SIZE') {
    try {
  ...
  // applyQuantityFilter(quantity: number, filters: any[]) {
  ...
  hasInvalidParams(strategy: 'BBS' | 'BSS', crossRate: number, symbols: any[]) {
   ...
   */
  isFiltersValid(strategy: string, price: number, quantity: number, filters: any[]): boolean {
    try {
  ...
  runFilterError(name: string, error: string) {
   ...
   * @returns newValue
   */
  handleStepSize(value: number, stepSize: number) {
   ...
   * @returns quantity
   */
  handleTickSize(quantity: number, tickSize: number) {
  ...
  async execution(oportunity: any) {
    try {
      ...
      if (process.env.NODE_ENV?.includes('dev')) {
        ...
        if (!order1) throw new Error('Ordem 1 não foi executada.');
        ...
        if (order1.status === OrderStatusEnum.FILLED) {
          ...
          const secondTransactionType = (oportunity.strategy === 'BBS') ? OrderSideEnum.BUY : OrderSideEnum.SELL
          const newOrder2 = {
            type: OrderTypeEnum.LIMIT,
          ...
          if (!order2) throw new Error('Ordem 2 não foi executada.');
          ...
          if (order2.status === OrderStatusEnum.FILLED) {
            ...
            if (!order3) throw new Error('Ordem 3 não foi executada.');
  ...
  async runError(status: string, error: any, oportunity: any) {
  ...
  createWebSocket(callback: (event: any) => {}) {
